...
do
    ...
    if (F_A) then
        ...
        return Notify(nil, "Loaded", "Ayush or @yonecess admin is already loaded... use 'killscript' to kill", nil),
...
AddCommand("setprefix", {}, "changes your prefix", {"1"}, function(Caller, Args)
    ...
    if (match(PrefixToSet, "%A")) then
        ...
        return "use command saveprefix to save your prefix made by Ayush"
    else
...
AddCommand("config", {"conf"}, "shows ayush admin config", {}, function(Caller, Args, CEnv)
...
Utils.Notify(LocalPlayer, "Welcome", "'cmds' to see all of the commands, Made by Ayush/@yonecess");
if (debug.info(2, "f") == nil) then
    Utils.Notify(LocalPlayer, "Ayush Admin Script", "Made by Ayush", 10);
end
